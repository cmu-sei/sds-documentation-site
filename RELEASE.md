...
The SDS Documentation Site follows **semantic versioning** and uses a two-step release process:
...
1. Go to https://github.com/cmu-sei/sds-documentation-site/releases/new
...
If the script encounters any errors (git operations, network issues, etc.), it will attempt to restore your repository to its original state by reverting uncommitted changes to `package.json`.
...
```bash
npm install @cmu-sei/sds-documentation-site@beta
```

Or a specific beta version:
```bash
npm install @cmu-sei/sds-documentation-site@4.5.0-beta.a1b2c3d
```
